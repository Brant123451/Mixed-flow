...
Program Tracking_Capturing_Mixed_Flows
    ...
    Do
        ...
        if (EnableRuntimeStepPrint) Print*,x,Time,Inter_type
        x=x+1
        ...
        If(Time>2) Then
            Do i=1,n,1
                Write(3,*) (i-0.5)*Deltax,H(i)
            Enddo
        Endif
        
        !Write(3,*) Time,(Pg-P_ref)/1000
        If(Time>2) exit
            ...
            Else
                ...
                Else !转换界面
                    If((H(i-1)-y_ref)>0.and.(H(i)-y_ref)<0) Then !出现流态过渡,上游为压力流，下游为自由液面流
                        ...
                        !H_gas=H_gas-Deltat*1.4*(H_gas+Hb)/VolumeG*(-Q_gas)
                        !VolumeG=VolumeG+Deltat*(-Q_gas)
                    Endif
                Endif
            Endif
        Enddo
